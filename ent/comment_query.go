...
// QueryCommenter chains the current query on the "commenter" edge.
func (cq *CommentQuery) QueryCommenter() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, comment.CommenterTable, comment.CommenterColumn),
		)
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withCommenter: cq.withCommenter.Clone(),
...
// WithCommenter tells the query-builder to eager-load the nodes that are connected to
// the "commenter" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommentQuery) WithCommenter(opts ...func(*UserQuery)) *CommentQuery {
	query := &UserQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCommenter = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context) ([]*Comment, error) {
	...
	if query := cq.withCommenter; query != nil {
